...
public class TournamentModule : BaseCommandModule {
    ...
    [Description(
        "Moves all channels from a given category to another one and appends a prefix to the name of each channel.")]
    [Command("archivetournament")]
    [Aliases("archivet", "ac")]
    [RequireOwner]
    public async Task ArchiveTournament(CommandContext ctx, ulong sourceCategoryId, ulong destinationCategoryId,
        [RemainingText] string channelPrefix) {
